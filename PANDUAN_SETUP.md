# Panduan Setup — Media Pembelajaran Interaktif "Jenis Instrumen Investasi"
...
3. Isi:
   - **Name**: misalnya `media-instrumen-investasi`
...
Menu **Daftar Nilai** di sidebar menampilkan rekap seluruh skor evaluasi siswa (nama, kelas, jumlah benar, skor objektif, waktu mengerjakan), diambil langsung dari tabel `evaluasi_hasil` di Supabase, dan bisa difilter per kelas. Karena tabel ini bisa dibaca publik lewat anon key (lihat catatan keamanan di bawah), siapa pun yang membuka link media ini juga bisa melihat daftar nilai — cocok untuk transparansi kelas, tapi beri tahu ini kepada siswa jika perlu.
